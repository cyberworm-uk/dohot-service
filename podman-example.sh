...
podman pull ghcr.io/cyberworm-uk/dnscrypt-proxy:latest || fail_out "Unable to pull doh-proxy"
podman pull ghcr.io/cyberworm-uk/tor-client:latest || fail_out "Unable to pull tor-proxy"
...
if [[ $? -eq 1 ]]; then
  ...
  podman run --rm --name dohot-torproxy \
    ...
    -d ghcr.io/cyberworm-uk/tor-client:latest || fail_out "Unable to run tor-proxy"
  podman run --rm --name dohot-dohproxy \
    --label "io.containers.autoupdate=registry" \
    --pod dohot \
    -d ghcr.io/cyberworm-uk/dnscrypt-proxy:latest || fail_out "Unable to run doh-proxy"
